...
def simplified_data(num_train, num_dev, num_test):
    ...
    random.seed(0)
    train_trees = loadTrees('train')+loadTrees('test')
    real_test = loadTrees('testing_data')
    ...
    train = train_trees[:num_train]
    dev = train_trees[num_train : num_train+num_dev]
    test = train_trees[num_train+num_dev : num_train+num_dev+num_test]
    ...
    return train, dev, test, real_test
